...
const translations = {
  th: {
    ...
    createParty: "สร้างปาร์ตี้ / แชร์บิล", createPartyHint: "ใส่วันที่หรือไม่ก็ได้ เหมาะกับมื้อเดียว งานเดียว หรือคำนวณด่วน",
    searchPlaceholder: "ค้นหาทริป จังหวัด ร้าน หรือโรงแรม...",
    emptyHome: "ยังไม่มีทริปหรือปาร์ตี้ — เริ่มจากปุ่มด้านบนได้เลย",
  ...
  en: {
    ...
    createParty: "New Party / Split Bill", createPartyHint: "Date is optional — great for a single meal or a quick calculation",
    searchPlaceholder: "Search trips, province, restaurant, or hotel...",
    emptyHome: "No trips or parties yet — start with a button above",
...
function computeHouseholdNet(members, households, entries) {
  ...
  members.forEach((m) => {
    ...
    householdNet[rep] = round2((householdNet[rep] || 0) + (individualNet[m] || 0));
  });
  return householdNet;
}
...
// ================= HOME =================
function Home({ data, query, setQuery, onNewTrip, onNewParty, onOpenTrip, onOpenParty, onOpenTrash }) {
  const { t, lang } = useT();
    ...
    <div>
      <div className="ps-create-grid">
        <button className="ps-create-card create-party" onClick={onNewParty}>
          <Users size={20} /><span>{t("createParty")}</span><small>{t("createPartyHint")}</small>
        </button>
        <button className="ps-create-card create-trip" onClick={onNewTrip}>
          <MapPin size={20} /><span>{t("createTrip")}</span><small>{t("createTripHint")}</small>
        </button>
      </div>

      <div className="ps-trash-entry-row">
        <button className="ps-btn-ghost" onClick={onOpenTrash}>
...
function SplitPicker({ members, selected, toggle }) {
  ...
  return (
    <>
      <label className="ps-label">{lang === "th" ? "หารกับใคร" : "Split with"}</label>
      <p className="ps-hint">
        {lang === "th"
...
function MemberManagerPanel({ members, households, isReferenced, onAddMember, onRemoveMember, onCreateHousehold, onDissolveHousehold }) {
  const { t, lang } = useT();
  ...
  if (!open) {
    return (
      <button className="ps-btn-ghost" style={{ marginBottom: 14 }} onClick={() => setOpen(true)}>
  ...
  return (
    <TornCard>
      ...
      <label className="ps-label">{lang === "th" ? "สมาชิกทั้งหมด" : "All members"}</label>
      <div className="ps-chip-row">
        {members.map((m) => (
      ...
      <HouseholdGroupSection members={members} households={households} onCreateHousehold={onCreateHousehold} onDissolveHousehold={onDissolveHousehold} />
...
// ================= NEW TRIP FORM =================
function HouseholdGroupSection({ members, households, onCreateHousehold, onDissolveHousehold }) {
  const { lang } = useT();
  ...
  const unassigned = members.filter((m) => !households.some((h) => h.memberNames.includes(m)));
  const toggleGroupMember = (m) => setGroupSelection((p) => (p.includes(m) ? p.filter((x) => x !== m) : [...p, m]));
  const submitGroup = () => {
    if (groupSelection.length < 2 || !groupRep) return;
    onCreateHousehold({ id: uid("hh"), name: groupName.trim() || groupRep, memberNames: groupSelection, representative: groupRep });
    setCreatingGroup(false); setGroupName(""); setGroupSelection([]); setGroupRep("");
  };
  ...
  return (
    <div style={{ marginTop: 14 }}>
      <label className="ps-label">{lang === "th" ? "จับคู่ / จัดกลุ่ม (ไม่บังคับ)" : "Pair up / group members (optional)"}</label>
      <p className="ps-hint">
        {lang === "th"
      ...
      {households.map((h) => (
        <div key={h.id} className="ps-settle-item" style={{ marginBottom: 8 }}>
          <div>
            <b>{lang === "th" ? "ตัวแทน" : "Rep"}: {h.representative}</b>
            <div className="ps-ledger-meta">{lang === "th" ? "สมาชิก" : "Members"}: {h.memberNames.join(", ")}</div>
          </div>
          <button type="button" className="ps-ledger-del" style={{ marginLeft: "auto" }} onClick={() => onDissolveHousehold(h.id)}><Trash2 size={14} /></button>
        </div>
      ))}
      {!creatingGroup ? (
        unassigned.length >= 2 && (
          <button type="button" className="ps-btn-ghost" onClick={() => setCreatingGroup(true)}><Plus size={13} /> {lang === "th" ? "จับคู่ / สร้างกลุ่มใหม่" : "New pair / group"}</button>
        )
      ) : (
        <div style={{ borderTop: "1px dashed var(--line)", paddingTop: 12, marginTop: 8 }}>
          ...
          <input className="ps-input" placeholder={lang === "th" ? "เช่น ครอบครัว A" : "e.g. Family A"} value={groupName} onChange={(e) => setGroupName(e.target.value)} />
          ...
          <div className="ps-form-actions">
            <button type="button" className="ps-btn" disabled={groupSelection.length < 2 || !groupRep} onClick={submitGroup}><Check size={14} /> {lang === "th" ? "สร้างกลุ่ม" : "Create group"}</button>
            <button type="button" className="ps-btn-ghost" onClick={() => { setCreatingGroup(false); setGroupSelection([]); setGroupRep(""); }}>{lang === "th" ? "ยกเลิก" : "Cancel"}</button>
          </div>
        </div>
      )}
    </div>
...
// ================= TRIP DETAIL =================
function TripDetail({ trip, onBack, onUpdate, onDelete }) {
  const { t, lang } = useT();
  ...
  return (
    <div>
      ...
      {readOnly && <ClosedBanner onReopen={() => onUpdate((t) => ({ ...t, closed: false, closedAt: null }))} />}
            ...
            {trip.accommodations.slice().sort((a, b) => (a.night || "").localeCompare(b.night || "")).map((a) => (
              <li key={a.id}>
                <div style={{ flex: 1 }}>
                  ...
                  <div className="ps-ledger-meta">{fmtDate(a.night)} · {lang === "th" ? `${a.paidBy} จ่าย · หาร ${a.splitAmong.length} คน` : `${a.paidBy} paid · split ${a.splitAmong.length}`}</div>
                </div>
          ...
          {mode === "editMeal" ? (
            ...
            <>
              ...
              <p className="ps-ledger-meta">
                {fmtDate(meal.date)} · {mealLabel} · {lang === "th" ? `${meal.paidBy} จ่าย` : `${meal.paidBy} paid`}
            ...
            <div key={m} style={{ borderBottom: "1px dashed var(--line)", padding: "10px 0" }}>
              ...
              <div className="ps-ledger-meta" style={{ marginTop: 4 }}>
                {lang === "th"
                  ...
                  : `Total spent ฿${fmt(shareTotal)} (lodging ฿${fmt(b.accommodation)} · food ฿${fmt(b.food)} · beer ฿${fmt(b.beer)} · liquor ฿${fmt(b.liquor)}) · actually paid ฿${fmt(b.paid)}`}
              </div>
            </div>
          );
        })}
...
function PartyDetail({ party, onBack, onUpdate, onDelete }) {
  const { t, lang } = useT();
  ...
  return (
    <div>
      ...
      {readOnly && <ClosedBanner onReopen={() => onUpdate((p) => ({ ...p, closed: false }))} />}
        ...
        {party.expenses.length === 0 ? (
          ...
          <ul className="ps-ledger">
            {party.expenses.map((e) => (
              <li key={e.id}>
                <div style={{ flex: 1 }}><div className="ps-ledger-desc">{e.desc}</div><div className="ps-ledger-meta">{lang === "th" ? `${e.paidBy} จ่าย · หาร ${e.splitAmong.length} คน` : `${e.paidBy} paid · split ${e.splitAmong.length}`}</div></div>
...
function GlobalStyle() {
  return (
    <style>{`
      ...
      .ps-create-card.create-party { background: var(--gold); }
      .ps-create-card.create-trip { background: var(--jade); }
